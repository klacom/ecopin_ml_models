"""
Experiment 3.2 — Lightweight Binary Classifiers on Frozen CLIP Embeddings
==========================================================================
Methodology identical to Experiment 3.1 (train_exp03_clip_classifiers.py).
Only input embeddings and output directory differ.

Pipeline : Frozen CLIP ViT-B/32  ->  512-D embeddings  ->  classifier
Input    : c:/dev/datasets/ecopin_dataset/embeddings/exp03_2/clip_vitb32_{train,val,test}.npz
Output   : c:/dev/ecopin_ml_models/image_validation/artifacts/exp03_2_clip_refined/

Three classifiers:
  1. Linear   -- 512 -> 1  (BCEWithLogitsLoss, no hidden layer)
...
Does NOT overwrite Experiment 3.1 artifacts at:
  artifacts/exp03_clip_classifiers/
"""

import csv, json, random, sys, warnings
...
OUT_DIR = Path(r"C:\dev\ecopin_ml_models\image_validation\artifacts\exp03_2_clip_refined")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# Guard: confirm Exp 3.1 artifacts are separate and untouched
EXP31_ART = Path(r"C:\dev\ecopin_ml_models\image_validation\artifacts\exp03_clip_classifiers")
assert EXP31_ART.exists(), "Exp 3.1 artifact directory missing"
assert OUT_DIR.resolve() != EXP31_ART.resolve(), "Output must not be Exp 3.1 directory"

# ── HYPERPARAMETERS (identical to Exp 3.1) ────────────────────────────────────
SEED         = 42
BATCH_SIZE   = 64
MAX_EPOCHS   = 200
LR           = 1e-3
...
def load_split(name):
    ...
    return (
        torch.tensor(d["embeddings"], dtype=torch.float32),
        torch.tensor(d["label_ints"], dtype=torch.float32),
        d["filenames"].tolist(),
    )

print("Loading Exp 3.2 embeddings …")
X_train, y_train, fns_train = load_split("train")
X_val,   y_val,   fns_val   = load_split("val")
X_test,  y_test,  fns_test  = load_split("test")

assert X_train.shape == (840, EMB_DIM)
assert X_val.shape   == (180, EMB_DIM)
assert X_test.shape  == (180, EMB_DIM)
...
# Class-weighted loss (identical formula to Exp 3.1)
pos_weight = torch.tensor([y_train.shape[0] / y_train.sum() * 0.5]).to(device)
criterion  = nn.BCEWithLogitsLoss(pos_weight=pos_weight)

# ── ARCHITECTURES (identical to Exp 3.1) ──────────────────────────────────────
...
# ── EVALUATION (identical to Exp 3.1) ─────────────────────────────────────────
def evaluate(model, X, y_true_tensor, split_name):
    model.eval()
    with torch.no_grad():
...
# ── TRAINING LOOP (identical to Exp 3.1) ──────────────────────────────────────
def train_model(name, model_cls):
    print(f"\n{'='*60}")
    ...
    best_macro_f1, best_epoch, patience_count = -1.0, 0, 0
    history = []
        ...
        if val_macro_f1 > best_macro_f1:
            ...
            if patience_count >= PATIENCE:
                print(f"  Early stop at epoch {epoch} "
                      f"(best epoch {best_epoch}, best val mF1={best_macro_f1:.4f})")
                break
    ...
    for res in [val_results, test_results]:
        ...
        print(f"    invalid  P/R/F1 : {res['precision_invalid']:.4f} / "
              f"{res['recall_invalid']:.4f} / {res['f1_invalid']:.4f}")
        print(f"    valid    P/R/F1 : {res['precision_valid']:.4f} / "
              f"{res['recall_valid']:.4f} / {res['f1_valid']:.4f}")
    ...
    with open(artifact_dir / "config.json",       "w") as f: json.dump(config,       f, indent=2)
    with open(artifact_dir / "val_results.json",  "w") as f: json.dump(val_results,  f, indent=2)
    with open(artifact_dir / "test_results.json", "w") as f: json.dump(test_results, f, indent=2)
    ...
    return {"name": name, "best_epoch": best_epoch,
            "val_results": val_results, "test_results": test_results, "config": config}
...
# ── COMPARISON TABLE ──────────────────────────────────────────────────────────
print(f"\n{'='*70}")
print("EXP 3.2  COMPARISON TABLE")
print(f"{'='*70}")
...
for name, res in all_results.items():
    v = res["val_results"]
    t = res["test_results"]
    print(f"{name:<14} | {v['accuracy']:>8.4f} {v['macro_f1']:>8.4f} | "
          f"{t['accuracy']:>9.4f} {t['macro_f1']:>9.4f}")
...
# ── EXP 3.1 vs 3.2 COMPARISON ────────────────────────────────────────────────
EXP31_SUMMARY = Path(r"C:\dev\ecopin_ml_models\image_validation\artifacts"
                     r"\exp03_clip_classifiers\comparison_summary.json")
if EXP31_SUMMARY.exists():
    with open(EXP31_SUMMARY) as f:
        s31 = json.load(f)
    print(f"\n{'='*70}")
    print("EXP 3.1 vs EXP 3.2 — MACRO-F1 COMPARISON")
    print(f"{'='*70}")
    print(f"{'Model':<14} | {'3.1 Val':>8} {'3.2 Val':>8} {'Delta':>7} | "
          f"{'3.1 Test':>9} {'3.2 Test':>9} {'Delta':>7}")
    print("-" * 72)
    for name in ["linear", "small_mlp", "reg_mlp"]:
        v31   = s31["models"][name]["val_macro_f1"]
        t31   = s31["models"][name]["test_macro_f1"]
        v32   = all_results[name]["val_results"]["macro_f1"]
        t32   = all_results[name]["test_results"]["macro_f1"]
        dv    = v32 - v31
        dt    = t32 - t31
        print(f"{name:<14} | {v31:>8.4f} {v32:>8.4f} {dv:>+7.4f} | "
              f"{t31:>9.4f} {t32:>9.4f} {dt:>+7.4f}")
...
# Confirm Exp 3.1 artifacts are intact
exp31_checkpoints = list(EXP31_ART.rglob("best.pt"))
print(f"\nExp 3.1 artifacts intact: {len(exp31_checkpoints)} checkpoint(s) present at {EXP31_ART}")
