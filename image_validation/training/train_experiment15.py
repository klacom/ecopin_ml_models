"""
train_experiment15.py — EfficientNet-B0 Experiment 15

Experiment 15: Stage 2 LR Pilot (Seed 42)
Goal: Investigate training stability by reducing the Stage 2 learning rate
from 1e-4 to 5e-5 with the no-rotation Config A configuration.

CHANGES FROM CORRECTED BASELINE (Seed 42, No Rotation, Stage 2 LR = 1e-4):
  1. LR_STAGE2 = 5e-5 (was 1e-4)
...
UNCHANGED FROM CORRECTED BASELINE:
  ...
  - NO RandomRotation
  - Dataset splits, Preprocessing, Class weights, Batch size (16)
...
ARTIFACT_ROOT   = os.path.join(PROJECT_ROOT, 'artifacts', 'exp15_efficientnet_b0')
CHECKPOINT_ROOT = os.path.join(PROJECT_ROOT, 'checkpoints', 'exp15')
...
BATCH_SIZE      = 16
LR_STAGE1       = 1e-3
LR_STAGE2       = 5e-5       # CHANGE: Pilot Stage 2 LR (was 1e-4)
PATIENCE        = 5
...
def get_transform(train: bool = False):
    ...
    Training: RandomHorizontalFlip(p=0.2) only (NO RandomRotation).
...
# ── Macro F1 Early Stopping (Experiment 15 Protocol) ─────────
...
def train():
    ...
    config = {
        'experiment':              'exp15_efficientnet_b0',
        ...
        'changes_vs_historical_baseline': [
            '[PROTOCOL] Stage 2 LR changed from 1e-4 to 5e-5',
            '[PROTOCOL] Checkpoint selection metric: Macro F1',
        ],
        'unchanged_vs_historical_baseline': [
            'Seed = 42',
            'Model, Dataset, Preprocessing, Class weights, Loss, Optimizer, Scheduler, Batch size, Patience',
            'Augmentation: RandomHorizontalFlip(p=0.2) only (No Rotation)',
    ...
    if n_errors > 0:
        ...
        for k, e in enumerate(sorted(errors, key=lambda x: (x['true_label'], x['image_id']))):
            ...
            ax.axis('off')
        plt.suptitle(f'Experiment 15 Misclassifications ({n_errors} images)', fontsize=11)
        plt.tight_layout()
    ...
    report_md = f"""# EfficientNet-B0 Experiment 15 — Validation Report
...
| Item | Corrected Baseline (Seed 42) | Experiment 15 | Type |
|------|-----------------------------|---------------|------|
| Stage 2 LR | 1e-4 | **5e-5** | Learning Rate Pilot |
| Metric | val_loss | **Macro F1** | Protocol update |

## Unchanged from Corrected Baseline

Seed = 42, Augmentations (H-Flip 0.2 ONLY, No Rotation), Dataset, Model, Preprocessing, Class weights, Loss, Optimizer, Scheduler, Batch size, Max epochs, Patience.
...
All artifacts: `c:/dev/ecopin_image_validation/artifacts/exp15_efficientnet_b0/`
"""
