"""
train_experiment8.py — EfficientNet-B0 Experiment 8

Experiment 8: Seed 123 Control (No Rotation)
Goal: Serve as the matched Seed 123 baseline to correctly evaluate the within-seed
effect of rotation when compared to Experiment 7 (Seed 123, Rotation 15°).

CHANGES FROM CORRECTED BASELINE (Seed 42, No Rotation):
  1. SEED = 123 (was 42)
  2. Checkpoint selection metric: Macro F1 (maximize)
     (Tie-breakers: val_accuracy, then val_loss)

UNCHANGED FROM CORRECTED BASELINE:
  - RandomHorizontalFlip(p=0.2) (exactly as in corrected baseline)
  - NO RandomRotation
...
ARTIFACT_ROOT   = os.path.join(PROJECT_ROOT, 'artifacts', 'exp8_efficientnet_b0')
CHECKPOINT_ROOT = os.path.join(PROJECT_ROOT, 'checkpoints', 'exp8')
...
# ── Macro F1 Early Stopping (Experiment 8 Protocol) ─────────
...
def train():
    ...
    config = {
        'experiment':              'exp8_efficientnet_b0',
        ...
        'changes_vs_historical_baseline': [
            '[PROTOCOL] Seed changed from 42 to 123',
    ...
    if n_errors > 0:
        ...
        for k, e in enumerate(sorted(errors, key=lambda x: (x['true_label'], x['image_id']))):
            ...
            ax.axis('off')
        plt.suptitle(f'Experiment 8 Misclassifications ({n_errors} images)', fontsize=11)
        plt.tight_layout()
    ...
    report_md = f"""# EfficientNet-B0 Experiment 8 — Validation Report
...
| Item | Corrected Baseline (Seed 42) | Experiment 8 (Seed 123) | Type |
|------|-----------------------------|-------------------------|------|
| Seed | 42 | **123** | Seed Reliability Test |
| Metric | val_loss | **Macro F1** | Protocol update |

## Unchanged from Corrected Baseline

Augmentations (H-Flip 0.2 ONLY, No Rotation), Dataset, Model, Preprocessing, Class weights, Loss, Optimizer, Scheduler, Batch size, Max epochs, Patience.
...
All artifacts: `c:/dev/ecopin_image_validation/artifacts/exp8_efficientnet_b0/`
"""
