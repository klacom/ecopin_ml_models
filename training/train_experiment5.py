"""
train_experiment5.py — EfficientNet-B0 Experiment 5

Experiment 5: Stronger Horizontal-Flip-Only Ablation
Goal: Isolate the effect of increasing horizontal-flip probability (0.2 -> 0.5),
without RandomRotation, against the Corrected Baseline.

CHANGES FROM CORRECTED BASELINE:
  1. RandomHorizontalFlip(p=0.2) -> RandomHorizontalFlip(p=0.5)
  2. Checkpoint selection metric: Macro F1 (maximize)
     (Tie-breakers: val_accuracy, then val_loss)

UNCHANGED FROM CORRECTED BASELINE:
  - No RandomRotation (absent in both CB and Exp 5)
  - Stage 1 freeze: 5 epochs (epochs 1-5 frozen, 6+ unfrozen)
  - Dataset splits, Seed (42), Preprocessing, Class weights, Batch size (16)
  - Model: EfficientNet-B0, ImageNet pretrained
  - Loss: CrossEntropyLoss (no label smoothing)
  - Optimizer: Adam (LR 1e-3 -> 1e-4)
  - Scheduler: ReduceLROnPlateau(patience=2) monitoring val_loss
  - Early stopping patience: 5
  - No ColorJitter, No Weight Decay, No Dropout
...
ARTIFACT_ROOT   = os.path.join(PROJECT_ROOT, 'artifacts', 'exp5_efficientnet_b0')
CHECKPOINT_ROOT = os.path.join(PROJECT_ROOT, 'checkpoints', 'exp5')
...
def get_transform(train: bool = False):
    ...
    Training: RandomHorizontalFlip(p=0.5) only (no RandomRotation).
...
# ── Macro F1 Early Stopping (Experiment 5 Protocol) ─────────
class MacroF1EarlyStopping:
    """Monitors Macro F1 (maximize).
    Tie-breaker: higher val_accuracy; if still tied, lower val_loss.
    """
    ...
    def __call__(self, epoch: int, macro_f1: float, val_acc: float,
                 val_loss: float, model):
...
def train():
    ...
    # Checkpoint selection: Macro F1 (maximize)
    early_stop = MacroF1EarlyStopping(
        patience=PATIENCE,
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        # ── Early stopping: Macro F1 criterion (Experiment 4 Protocol) ─────────
    ...
    # Best checkpoint = epoch with max Macro F1
    print(f'\nReloading best checkpoint (epoch {early_stop.best_epoch}, '
          f'selected by Macro F1={early_stop.best_macro_f1:.4f}) ...')
    ...
    config = {
        'experiment':              'exp5_efficientnet_b0',
        ...
        'early_stopping_metric':   'Macro F1 (maximize); tie-breaker: val_acc, val_loss',
        'checkpoint_note':         'per-epoch checkpoints saved; best selected by Macro F1',
        'changes_vs_corrected_baseline': [
            '[AUGMENTATION] RandomHorizontalFlip: p=0.2 -> p=0.5',
            '[PROTOCOL] Checkpoint metric: val_loss -> Macro F1 (maximize)',
        ],
        'unchanged_vs_corrected_baseline': [
            'No RandomRotation (absent in both CB and Exp 5)',
            'Stage 1: 5 frozen epochs',
            'Model, Dataset, Preprocessing, Seed, Class weights, Loss, Optimizer, Scheduler, Batch size, Patience',
        ],
        ...
        'augmentation_train':      ['RandomHorizontalFlip(p=0.5)', 'pad_to_square',
    ...
    if n_errors > 0:
        ...
        for k, e in enumerate(sorted(errors, key=lambda x: (x['true_label'], x['image_id']))):
            ...
            ax.axis('off')
        plt.suptitle(f'Experiment 5 Misclassifications ({n_errors} images)', fontsize=11)
        plt.tight_layout()
    ...
    report_md = f"""# EfficientNet-B0 Experiment 4 — Validation Report

**Experiment:** `exp4_efficientnet_b0`
...
| Item | Corrected Baseline | Experiment 4 | Type |
|------|--------------------|--------------|------|
...
Model, Dataset, Preprocessing, Seed, Class weights, Loss, Optimizer, Scheduler, Batch size, Max epochs, Patience.

## Validation Metrics (best checkpoint — epoch {early_stop.best_epoch})
...
| Train−Val gap | {train_acc_at_best - val_acc_b:.4f} |
...
All artifacts: `c:/dev/ecopin_image_validation/artifacts/exp4_efficientnet_b0/`
"""
