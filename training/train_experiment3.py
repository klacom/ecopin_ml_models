"""
train_experiment3.py — EfficientNet-B0 Experiment 3

Experiment 3: Spatial Augmentation Ablation
Goal: Isolate the impact of spatial augmentations against the Corrected Baseline.

CHANGES FROM CORRECTED BASELINE:
  1. RandomHorizontalFlip(p=0.5) (was 0.2)
  2. Add RandomRotation(degrees=15)
  3. Checkpoint selection metric: Macro F1 (maximize)
...
# ── Paths ─────────────────────────────────────────────────────────────────────────────────────────
ARTIFACT_ROOT   = os.path.join(PROJECT_ROOT, 'artifacts', 'exp3_efficientnet_b0')
CHECKPOINT_ROOT = os.path.join(PROJECT_ROOT, 'checkpoints', 'exp3')
BEST_CKPT_PATH  = os.path.join(CHECKPOINT_ROOT, 'best.pt')
os.makedirs(ARTIFACT_ROOT,   exist_ok=True)
os.makedirs(CHECKPOINT_ROOT, exist_ok=True)

# ── Hyperparameters (identical to historical baseline except where noted) ────────────────────────
SEED            = 42
MAX_EPOCHS      = 20
...
# ── Preprocessing (IDENTICAL to historical baseline) ──────────────────────────────────────────────
...
def get_transform(train: bool = False):
    ...
    Training: RandomHorizontalFlip(p=0.5) and RandomRotation(15) added.
...
# ── Macro F1 Early Stopping (Experiment 3 Protocol) ─────────
...
# ── Per-epoch validation helper ──────────────────────────────────────────────────────────────────
def run_validation(model, val_loader, criterion):
    """Run full validation pass.
    criterion is passed explicitly — no global state dependency.
...
def plot_confusion_matrix(cm, class_names, output_path, title='Experiment 3 — Validation Confusion Matrix'):
...
def train():
    ...
    # ── Loss (UNCHANGED: CrossEntropyLoss, class-weighted, no label smoothing) ──
    criterion = nn.CrossEntropyLoss(weight=class_weights)

    # ── Model (UNCHANGED: timm EfficientNet-B0, ImageNet pretrained, 4 classes)
    ...
    # ── Stage 1: freeze backbone (UNCHANGED logic; duration corrected to 5) ────
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        # ── Save per-epoch checkpoint (observability — does not affect selection)
        epoch_ckpt_path = os.path.join(CHECKPOINT_ROOT, f'epoch_{epoch:02d}.pt')
        torch.save(model.state_dict(), epoch_ckpt_path)

        # ── Save per-epoch metrics (observability) ────────────────────────────────────────────────
        epoch_metrics[epoch] = {
            'val_loss':    val_loss,
            'val_acc':     val_acc,
            'macro_f1':    macro_f1,
        ...
        # ── Scheduler step: monitors val_loss (IDENTICAL to historical) ──────────
        scheduler.step(val_loss)

        # ── Early stopping: Macro F1 criterion (Experiment 3 Protocol) ─────────
        early_stop(epoch, macro_f1, val_acc, val_loss, model)
        if early_stop.early_stop:
            print(f'Early stopping triggered at epoch {epoch}')
            break

        # ── Stage transition (fires at END of epoch STAGE1_EPOCHS) ──────────────
    ...
    config = {
        ...
        'early_stopping_metric':   'Macro F1 (maximize); tie-breaker: val_acc, then val_loss',
        'checkpoint_note':         'Experiment 3 protocol',
