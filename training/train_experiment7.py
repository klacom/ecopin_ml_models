"""
train_experiment7.py — EfficientNet-B0 Experiment 7

Experiment 7: Seed Reliability Test
Goal: Isolate the impact of the random seed by running exactly Exp4 (15° rotation)
but with SEED = 123.
...
UNCHANGED FROM EXPERIMENT 4:
  ...
  - Checkpoint selection metric: Macro F1 (maximize)
...
ARTIFACT_ROOT   = os.path.join(PROJECT_ROOT, 'artifacts', 'exp7_efficientnet_b0')
CHECKPOINT_ROOT = os.path.join(PROJECT_ROOT, 'checkpoints', 'exp7')
...
def get_transform(train: bool = False):
    ...
    Training: RandomHorizontalFlip(p=0.2) and RandomRotation(15) added.
...
# ── Macro F1 Early Stopping (Experiment 7 Protocol) ─────────
...
def train():
    ...
    config = {
        'experiment':              'exp7_efficientnet_b0',
        ...
        'unchanged_vs_experiment4': [
            ...
            'Augmentation: RandomHorizontalFlip(p=0.2), RandomRotation(degrees=15)',
    ...
    if n_errors > 0:
        ...
        for k, e in enumerate(sorted(errors, key=lambda x: (x['true_label'], x['image_id']))):
            ...
            ax.axis('off')
        plt.suptitle(f'Experiment 7 Misclassifications ({n_errors} images)', fontsize=11)
        plt.tight_layout()
    ...
    report_md = f"""# EfficientNet-B0 Experiment 7 — Validation Report
...
| Item | Experiment 4 | Experiment 7 | Type |
|------|--------------|--------------|------|
| Seed | 42 | **123** | Seed Reliability Test |

## Unchanged from Experiment 4

Augmentations (Rotation 15°, H-Flip 0.2), Metric (Macro F1), Dataset, Model, Preprocessing, Class weights, Loss, Optimizer, Scheduler, Batch size, Max epochs, Patience.
...
All artifacts: `c:/dev/ecopin_image_validation/artifacts/exp7_efficientnet_b0/`
"""
