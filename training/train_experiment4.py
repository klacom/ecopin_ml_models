"""
train_experiment4.py — EfficientNet-B0 Experiment 4

Experiment 4: Isolated Rotation-only Ablation
Goal: Isolate the impact of rotation against the Corrected Baseline.

CHANGES FROM CORRECTED BASELINE:
  1. Add RandomRotation(degrees=15)
...
ARTIFACT_ROOT   = os.path.join(PROJECT_ROOT, 'artifacts', 'exp4_efficientnet_b0')
CHECKPOINT_ROOT = os.path.join(PROJECT_ROOT, 'checkpoints', 'exp4')
...
# ── Macro F1 Early Stopping (Experiment 4 Protocol) ─────────
...
def train():
    ...
    if n_errors > 0:
        ...
        for k, e in enumerate(sorted(errors, key=lambda x: (x['true_label'], x['image_id']))):
            ...
            ax.axis('off')
        plt.suptitle(f'Experiment 4 Misclassifications ({n_errors} images)', fontsize=11)
        plt.tight_layout()
