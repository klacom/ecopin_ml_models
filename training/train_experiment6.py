"""
train_experiment6.py — EfficientNet-B0 Experiment 6

Experiment 6: Rotation Degree Ablation (10° vs 15°)
Goal: Determine whether RandomRotation(10°) provides similar or better
performance compared to Exp4's RandomRotation(15°).

CHANGES FROM EXPERIMENT 4:
  1. RandomRotation(degrees=15) → RandomRotation(degrees=10)
  2. Checkpoint selection metric: Macro F1 (maximize)
     (Tie-breakers: val_accuracy, then val_loss)

UNCHANGED FROM EXPERIMENT 4 (= Corrected Baseline except Rotation degree):
...
ARTIFACT_ROOT   = os.path.join(PROJECT_ROOT, 'artifacts', 'exp6_efficientnet_b0')
CHECKPOINT_ROOT = os.path.join(PROJECT_ROOT, 'checkpoints', 'exp6')
...
def get_transform(train: bool = False):
    ...
    Training: RandomHorizontalFlip(p=0.2) and RandomRotation(10°).
...
# ── Macro F1 Early Stopping (Experiment 6 Protocol) ─────────
...
def train():
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        # ── Early stopping: Macro F1 criterion (Experiment 6 Protocol) ─────────
    ...
    config = {
        'experiment':              'exp6_efficientnet_b0',
        ...
        'augmentation_train':      ['RandomHorizontalFlip(p=0.2)', 'RandomRotation(10)', 'pad_to_square',
                                    'Resize(224,224)', 'ToTensor', 'Normalize(ImageNet)'],
        'augmentation_val':        ['pad_to_square', 'Resize(224,224)',
                                    'ToTensor', 'Normalize(ImageNet)'],
        'changes_vs_experiment4': [
            '[AUGMENTATION] RandomRotation: degrees=15 -> degrees=10',
        ],
        'unchanged_vs_experiment4': [
            'RandomHorizontalFlip(p=0.2)',
            'Stage 1: 5 frozen epochs', 'Macro F1 checkpoint selection',
    ...
    if n_errors > 0:
        ...
        for k, e in enumerate(sorted(errors, key=lambda x: (x['true_label'], x['image_id']))):
            ...
            ax.axis('off')
        plt.suptitle(f'Experiment 6 Misclassifications ({n_errors} images)', fontsize=11)
        plt.tight_layout()
    ...
    report_md = f"""# EfficientNet-B0 Experiment 6 — Validation Report
...
## Changes vs. Experiment 4

| Item | Experiment 4 | Experiment 6 | Type |
|------|--------------|--------------|------|
| Rotation | RandomRotation(15°) | **RandomRotation(10°)** | Degree ablation |

## Unchanged from Experiment 4

H-flip p=0.2, Macro F1 checkpoint selection, Model, Dataset, Preprocessing, Seed, Class weights, Loss, Optimizer, Scheduler, Batch size, Max epochs, Patience.
...
All artifacts: `c:/dev/ecopin_image_validation/artifacts/exp6_efficientnet_b0/`
"""
