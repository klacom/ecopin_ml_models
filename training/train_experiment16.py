"""
train_experiment16.py — EfficientNet-B0 Experiment 16

Experiment 16: Stage 2 LR Pilot (Seed 123)
...
CHANGES FROM EXPERIMENT 8 (Seed 123, No Rotation, Stage 2 LR = 1e-4):
  1. LR_STAGE2 = 5e-5 (was 1e-4)

UNCHANGED FROM EXPERIMENT 8:
  - SEED = 123
...
ARTIFACT_ROOT   = os.path.join(PROJECT_ROOT, 'artifacts', 'exp16_efficientnet_b0')
CHECKPOINT_ROOT = os.path.join(PROJECT_ROOT, 'checkpoints', 'exp16')
...
# ── Macro F1 Early Stopping (Experiment 16 Protocol) ─────────
...
def train():
    ...
    config = {
        'experiment':              'exp16_efficientnet_b0',
        ...
        'changes_vs_experiment8': [
            '[PROTOCOL] Stage 2 LR changed from 1e-4 to 5e-5',
        ],
        'unchanged_vs_experiment8': [
            'Seed = 123',
            ...
            'Checkpoint selection metric: Macro F1',
        ],
    }
    ...
    if n_errors > 0:
        ...
        for k, e in enumerate(sorted(errors, key=lambda x: (x['true_label'], x['image_id']))):
            ...
            ax.axis('off')
        plt.suptitle(f'Experiment 16 Misclassifications ({n_errors} images)', fontsize=11)
        plt.tight_layout()
    ...
    report_md = f"""# EfficientNet-B0 Experiment 16 — Validation Report
...
## Changes vs. Experiment 8

| Item | Experiment 8 (Seed 123) | Experiment 16 | Type |
|------|-------------------------|---------------|------|
| Stage 2 LR | 1e-4 | **5e-5** | Learning Rate Pilot |

## Unchanged from Experiment 8

Seed = 123, Augmentations (H-Flip 0.2 ONLY, No Rotation), Dataset, Model, Preprocessing, Class weights, Loss, Optimizer, Scheduler, Batch size, Max epochs, Patience, Macro F1 Selection.
...
All artifacts: `c:/dev/ecopin_image_validation/artifacts/exp16_efficientnet_b0/`
"""
